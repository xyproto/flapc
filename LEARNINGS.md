...
Our code signature generation in `generateCodeSignature()` produces invalid signatures that macOS rejects. When comparing with Clang binaries, we're missing several load commands and the signature format appears incorrect.

### What We Know Works
- Clang-generated ARM64 binaries work fine
- Our binaries pass `otool` validation
- The Mach-O structure is mostly correct
- Tests pass because they only check file creation, not execution

### Investigation Status
Need to either:
1. Fix `generateCodeSignature()` to produce valid signatures
2. Use external `codesign` tool post-compilation  
3. Debug why codesign says "failed strict validation"

The binary is rejected during dyld loading phase, before any user code runs.
