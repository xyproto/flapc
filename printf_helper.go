...
// Signature: _flap_float_to_string(xmm0: float64, rdi: buffer_ptr) -> (rsi: str_start, rdx: length)
func EmitFloatToStringRuntime(eb *ExecutableBuilder, out *Out) error {
	if VerboseMode {
		fmt.Fprintf(os.Stderr, "DEBUG: Emitting _flap_float_to_string runtime function\n")
	}

	eb.MarkLabel("_flap_float_to_string")
	...
	if VerboseMode {
		fmt.Fprintf(os.Stderr, "DEBUG: _flap_float_to_string emitted successfully\n")
	}
