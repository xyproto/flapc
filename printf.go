...
func EmitPrintfRuntimeX86_64(eb *ExecutableBuilder, out *Out) error {
	if VerboseMode {
		fmt.Fprintf(os.Stderr, "DEBUG: Emitting _flap_printf for x86-64\n")
	}

	eb.MarkLabel("_flap_printf")
	...
	// rdi = format string pointer (Flap map)
	...
	if VerboseMode {
		fmt.Fprintf(os.Stderr, "DEBUG: _flap_printf emitted successfully\n")
	}
...
func EmitPrintfRuntimeARM64(eb *ExecutableBuilder, out *Out) error {
	if VerboseMode {
		fmt.Fprintf(os.Stderr, "DEBUG: Emitting _flap_printf for ARM64\n")
	}
	...
	if VerboseMode {
		fmt.Fprintf(os.Stderr, "DEBUG: ARM64 _flap_printf stub emitted\n")
	}
...
func EmitPrintfRuntimeRISCV64(eb *ExecutableBuilder, out *Out) error {
	if VerboseMode {
		fmt.Fprintf(os.Stderr, "DEBUG: Emitting _flap_printf for RISC-V 64\n")
	}
	...
	if VerboseMode {
		fmt.Fprintf(os.Stderr, "DEBUG: RISC-V _flap_printf stub emitted\n")
	}
