...
- If `main` returns `empty` or `[]`: exit code 0
...
import_source    = string_literal                        (* Quoted import for file paths *)
                 | bare_import_path [ "@" version_spec ] ; (* Unquoted import for packages/repos *)

bare_import_path = identifier { ( "." | "/" ) identifier }
                 | "git@" identifier ":" identifier "/" identifier ".git" ;
...
loop_statement  = "@" block
                | "@" identifier "in" ( expression | range_expr ) [ "max" expression ] block
...
assignment      = ( identifier | member_access | indexed_expr ) [ ":" type_annotation ] ( "=" | ":=" | "<-" ) expression
                | identifier ( "+=" | "-=" | "*=" | "/=" | "%=" | "**=" ) expression
                | identifier_list ( "=" | ":=" | "<-" ) expression ;  // Multiple assignment

member_access   = primary_expr "." ( identifier | integer ) ;
...
default_arm     = ( "~>" | "_" "=>" ) match_target ; // _ => is an alias for ~> for readability

match_target    = jump_target | expression ;

jump_target     = integer | "@" identifier ;

### Advanced Match: Jump Targets

For performance-critical code, such as interpreters or state machines, a `match` statement's target can be an integer literal, known as a `jump_target`. This is a low-level feature that functions like a `goto`, jumping to a correspondingly numbered label within the same function scope.

```flap
// Example of a simple interpreter loop
interpret = {
    ip := 0
    @ {
        opcode = bytecode[ip]
        ip {
            // Match on the instruction pointer (ip)
            // and jump to the corresponding label.
            OP_ADD => @add
            OP_SUB => @sub
            OP_HALT => ret
        }
    ...
    @add: {
        // ... add logic ...
        ip <- ip + 1
        ret @   // return to the top of the loop
    }
    @sub: {
        // ... sub logic ...
        ip <- ip + 1
...
```
**Warning:** This feature is intended for code generation and highly specialized use cases. It breaks normal control flow and should be avoided in general application code.
...
### Architecture-Specific `unsafe` Blocks

The `unsafe` statement can be followed by up to three blocks of code. This allows for platform-specific assembly code for different CPU architectures. The compiler will automatically select the correct block for the target architecture.
...
```flap
// Example of a multi-architecture unsafe block
unsafe {
    // x86_64 assembly goes here
} {
    // ARM64 assembly goes here
} {
    // RISC-V64 assembly goes here
}
```

If a block is omitted, it means that architecture is not supported for that specific `unsafe` operation. This is a powerful feature for writing highly optimized, cross-platform low-level code.
...
- `empty` – the canonical absence value (`{}`)
...
```
ret arena unsafe cstruct class as max this defer import export
```
...
| `<=`     | Receive (ENet)    | Receive from an ENet channel       | `msg <= &8080`                     |
...
Flap uses **NaN-boxing** to encode errors directly within `float64` values. This approach maintains the universal type system (`map[uint64]float64`) while enabling robust, zero-overhead error handling.

### Result Type Design

- **Success values:** Are represented by standard IEEE 754 `float64` numbers.
- **Error values:** Are represented by a Quiet NaN (`Not a Number`) where the 52-bit mantissa is used to store a 32-bit (4-char) error code.

**Error NaN Format:**
```
A 64-bit IEEE 754 float64 value:
[Sign][Exponent (11 bits)][Mantissa (52 bits)]

An error is encoded as follows:
[0][11111111111][1][...payload...][...error code (32 bits)...]
 |   |          |                |
 |   |          Quiet NaN bit    4-character ASCII error code
 |   All 1s, indicates NaN/Infinity
 Sign bit (0)
```

This design has several key properties:
- Errors are distinguishable from all valid floating-point numbers, including infinity and regular NaNs.
- Error checking is a single, efficient hardware instruction (`UCOMISD` on x86_64).
- There is zero runtime overhead for success cases; they are just standard numbers.
- The error codes are human-readable (e.g., "dv0 ", "mem ", "idx ").
...
- A plain `empty` value also returns `""` (no error).
...
The `or!` operator provides a default value or executes a block when the left side is an error or the `empty` value:
...
// Handle null pointers from C FFI, which return `empty` on failure
window := sdl.SDL_CreateWindow("Title", 640, 480, 0) or! {
...
ptr := c_malloc(1024) or! empty  // Returns `empty` if allocation failed
```

**Semantics:**
The `or!` operator triggers when its left-hand side is either a NaN-encoded error or the `empty` value.
1. Evaluate left operand.
2. Check if the value is a NaN-encoded error OR if the value is `empty`.
3. If it is an error or `empty`:
   - If the right side is a block: execute the block. The block's return value becomes the result.
   - If the right side is an expression: evaluate the expression. Its value becomes the result.
4. Otherwise (the value is valid): the result is the value of the left operand.
5. The right side is NOT evaluated unless the left side is an error or `empty` (short-circuit evaluation).
...
Use the `error` function to create a NaN-boxed error value:

```flap
// Create error with code "arg"
err = error("arg")

// Division by zero is a common way to generate a runtime error
fail = 0 / 0        // Returns a NaN-boxed error with code "dv0"
```
...
The `.error` accessor is implemented with a NaN check:
1. Check if the value is NaN. In x86_64, this is a single `UCOMISD` instruction which checks if a value is not equal to itself.
2. If it is NOT a NaN, it's a success value. Return `""`.
3. If it IS a NaN, extract the lower 32 bits of the mantissa. This contains the 4-byte ASCII error code.
4. Convert the code to a string, strip any trailing nulls or spaces, and return it.

### `or!` Implementation

The `or!` operator is implemented with checks for both NaN and `empty`:
1. Evaluate the left operand.
2. Check if the result is a NaN-boxed error.
3. If not, check if the result is `empty`.
4. If either check is true, evaluate and return the right operand.
5. Otherwise, return the value of the left operand.
